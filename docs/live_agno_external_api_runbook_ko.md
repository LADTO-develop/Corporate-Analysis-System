# Live Agno/Claude External API Runbook

작성일: 2026-05-21

## 문제 원인

현재 Stage 2 Agno 기본 모드는 `multi_llm_committee`이다. 이 모드는 역할별로 Claude, GPT, Gemini를 함께 쓰므로 `google-genai` 패키지와 `GOOGLE_API_KEY` 또는 `GEMINI_API_KEY`까지 필요하다.

이번 실험 목적은 Claude API + Agno 중심의 위원회 검토이므로, Gemini 의존성을 피하려면 `CAS_STAGE2_AGNO_MODE=single`로 실행한다. 이 모드에서는 `ANTHROPIC_API_KEY`와 Anthropic/Agno 패키지만 있으면 preflight가 통과한다.
...
따라서 실제 기업-회계연도 데이터를 Claude API로 보내는 live batch는 로컬 터미널에서 직접 실행한다. 아래 명령은 같은 저장소와 같은 Python 환경을 사용하므로 결과 파일 경로는 Codex에서 만든 산출물과 동일하게 남는다.
...
/usr/bin/env \
  ...
  CAS_STAGE2_MODEL_PROVIDER=anthropic \
...
## Claude API Live Batch

외부 뉴스/공시 API 없이 Claude/Agno 위원회만 확인할 때:
...
/usr/bin/env \
  ...
  --output-dir data/outputs/modeling/feature_43_xgboost/diagnostics/committee_review_holdout_unseen_agno_claude_live_batch \
  --policy balanced_current_45_or_near_threshold_0_10 \
  --per-category 2 \
  --max-cases 8 \
  --stage2-runner agno \
  --stage2-agno-mode single \
  --stage2-model-provider anthropic \
  --stage2-model claude-sonnet-4-5-20250929 \
  --workers 2
```
...
Gemini chair까지 포함하려면 다음이 필요하다.

```bash
/opt/anaconda3/envs/aura/bin/python -m pip install -e ".[agent]"
```

그리고 `.env`에 `GOOGLE_API_KEY` 또는 `GEMINI_API_KEY`를 추가한 뒤 `--stage2-agno-mode multi_llm_committee`로 실행한다.

## 속도 운영 원칙

Claude/Agno live batch는 deterministic guardrail보다 느리다. 실제 대시보드에서는 전체 기업을 보내지 않고 다음 조건 중 하나를 만족하는 기업만 live Agno로 보낸다.
