...
def _external_evidence_status_text(snapshot: dict[str, object] | None) -> str:
    """Return a user-facing status sentence for dashboard evidence collection."""
    if not snapshot:
        return (
            "선택 기업 기준으로 뉴스, 웹 검색, OpenDART 공시를 자동 조회해 "
            "2차 위원회 판단에 반영합니다."
        )
    status = str(snapshot.get("status") or "unknown")
    label = EXTERNAL_EVIDENCE_STATUS_LABELS.get(status, status)
    ...
    if status == "not_requested":
        return (
            "아직 외부 근거를 수집하지 않았습니다. 탭을 열면 뉴스/웹/OpenDART 결과를 자동으로 가져와 "
            "위원회 판단을 다시 계산합니다."
        )
    provider_text = _external_evidence_provider_status_text(snapshot.get("providers"))
    base = (
        f"{label}: 총 {item_count}개 근거를 확인했고, 직접 관련 {direct_count}개, "
        f"검증 기준 통과 {verified_count}개로 집계했습니다."
    )
    return f"{base} {provider_text}".strip()
...
def _external_veto_candidate_count(snapshot: dict[str, object] | None) -> int:
...
def _dashboard_veto_status_label(
    ...
    if bool(committee_view.get("veto_triggered", False)):
        return "발동"
    if _external_veto_candidate_count(evidence_snapshot) > 0:
        return "후보 검토"
    return "미발동"
...
def render_external_evidence_items(
    ...
    *,
    ...
    render_bold_value_block(summary_cols[0], "외부 근거", f"{total_count}건")
    render_bold_value_block(summary_cols[1], "직접 관련", f"{direct_count}건")
    render_bold_value_block(summary_cols[2], "검증 통과", f"{verified_count}건")
    render_bold_value_block(summary_cols[3], "강제 경고 후보", f"{veto_candidate_count}건")
...
def render_external_evidence_judgment(
    ...
    *,
    ...
    render_summary_banner("외부 근거를 이렇게 해석했어요", summary_text, summary_color)
    ...
    total_count = len(evidence_frame)
    caution_count = _external_veto_candidate_count(evidence_snapshot)
    ...
    render_badge_value_block(
        status_cols[0],
        "2차 위원회 판단",
        render_decision_badge(final_label),
    )
    render_bold_value_block(status_cols[1], "직접 관련 근거", f"{direct_count}/{total_count}건")
    render_bold_value_block(status_cols[2], "검증 통과 근거", f"{verified_count}건")
    render_badge_value_block(
        status_cols[3],
...
def render_committee_view_tab(
    *,
    ...
    veto_label = _dashboard_veto_status_label(committee_view, evidence_snapshot)
    ...
    if selected_output_format == "detailed":
        ...
        render_bold_value_block(
            detail_col3,
            "외부 근거 주의 후보",
            f"{_external_veto_candidate_count(evidence_snapshot)}건",
        )
