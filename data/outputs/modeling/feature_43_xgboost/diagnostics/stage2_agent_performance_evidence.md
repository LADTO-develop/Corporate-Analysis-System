...
남은 약점은 휴맥스처럼 실제 투자적격인 TN이 기준선 근처 또는 45개 보조 레이더 신호 때문에 보류로 올라가는 문제다. 다음 개선 guardrail은 다음 조건을 만족할 때 Agno/Claude가 보류로 올리는 것을 억제하는 방향이 적합하다.

- 1차 모델이 투자적격
- 투기등급 확률이 기준선 아래
- 직접적이고 검증된 외부 치명 리스크 없음
- 현금흐름/유동성/레버리지 중 최소 2개 이상이 방어적
