...
FEATURE_MASTER_PATH = INPUT_DIR / "feature_46_master.csv"
FEATURE_SPEC_PATH = INPUT_DIR / "feature_46_list.json"
CANONICAL_INFERENCE_PATH = INPUT_DIR / "feature_46_inference_2026.csv"
INFERENCE_AUX_PATH = ROOT / "data" / "raw" / "ts2000" / "feature_46_inference_2026_aux.csv"
...
def load_expected_columns() -> tuple[list[str], list[str]]:
    ...
    if missing_model_features:
        raise ValueError(
            "feature_46_list.json contains model features missing from inference columns: "
...
def add_official_derived_features(
    frame: pd.DataFrame,
    *,
    model_features: list[str],
) -> pd.DataFrame:
    """Materialize official generated model features before validation."""
    if not any(feature in model_features for feature in INDUSTRY_YEAR_AMOUNT_PCT_FEATURES):
        return frame
    missing_sources = [
        column for column in INDUSTRY_YEAR_AMOUNT_SOURCE_FEATURES if column not in frame.columns
    ]
    if missing_sources:
        raise ValueError(
            f"Industry-year percentile features require source columns: {missing_sources}"
        )
    output, _ = add_group_percentile_features(
        frame,
...
def main() -> None:
    ...
    inference = add_official_derived_features(inference, model_features=model_features)
