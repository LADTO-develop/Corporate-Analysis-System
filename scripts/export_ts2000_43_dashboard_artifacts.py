        ...
        rows.append(
            {
                ...
                "unit": info.get("unit", ""),
                "source": info.get("source", "ts2000_43_model_ready"),
                "note": info.get("note", ""),
            }
        )
...
def build_model_summary(
    ...
    return {
        "selected_model": "team43_xgboost",
        "dataset_name": "ts2000_43_model_ready",
        "test_overall_models": [
            {
                "model": "team43_xgboost",
                "rows": len(test_y),
...
def main() -> None:
    ...
    master = pd.read_csv(input_dir / "ts2000_43__master.csv", encoding="utf-8-sig")
    feature_json = read_json(input_dir / "ts2000_43_feature_list.json")
    metadata_json = read_json(args.metadata_path)
    metadata_columns = metadata_json.get("columns", [])

    train_ready = pd.read_csv(input_dir / "xgb_train_model_ready.csv", encoding="utf-8-sig")
    valid_ready = pd.read_csv(input_dir / "xgb_valid_model_ready.csv", encoding="utf-8-sig")
    test_ready = pd.read_csv(input_dir / "xgb_test_model_ready.csv", encoding="utf-8-sig")
    id_frames = {
    ...
    print(f"team43 dashboard artifacts written to: {output_dir}")
