...
from cas.modeling.size_context_features import add_group_percentile_features  # noqa: E402
...
FEATURE_SPEC_PATH = INPUT_DIR / "feature_46_list.json"
...
INDUSTRY_YEAR_AMOUNT_PCT_FEATURES = [
    "assets_total_industry_year_pct",
    "gross_profit_industry_year_pct",
    "depreciation_industry_year_pct",
]
INDUSTRY_YEAR_AMOUNT_SOURCE_FEATURES = ["assets_total", "gross_profit", "depreciation"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Rebuild the credit_46_features input tables from the latest TS2000 Model_V1 dataset."
        )
    )
    parser.add_argument(
        "--source",
        ...
        help="Latest labeled company-year dataset to use as the feature_46 source.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=INPUT_DIR,
        help="Directory where feature_46 master/split CSVs will be written.",
    )
...
def build_master_frame(source: pd.DataFrame, feature_spec: dict[str, object]) -> pd.DataFrame:
    ...
    generated_feature_columns = [
        feature for feature in INDUSTRY_YEAR_AMOUNT_PCT_FEATURES if feature in raw_feature_columns
    ]
    required_raw_feature_columns = [
        feature for feature in raw_feature_columns if feature not in generated_feature_columns
    ]
    required_columns = [*ID_COLUMNS, *required_raw_feature_columns, "is_speculative"]
    ...
    if any(feature in model_features for feature in INDUSTRY_YEAR_AMOUNT_PCT_FEATURES):
        missing_sources = [
            column
            for column in INDUSTRY_YEAR_AMOUNT_SOURCE_FEATURES
            if column not in master.columns
        ]
        if missing_sources:
            raise KeyError(
                f"Industry-year percentile features require source columns: {missing_sources}"
            )
        master, _ = add_group_percentile_features(
            master,
            group_columns=["fiscal_year", "industry_macro_category"],
            value_columns=INDUSTRY_YEAR_AMOUNT_SOURCE_FEATURES,
            suffix="industry_year",
        )

    # The 46-feature spec keeps categorical source values on the row while also
...
def write_outputs(
    ...
    master.to_csv(output_dir / "feature_46_master.csv", index=False, encoding="utf-8-sig")
...
def main() -> None:
    ...
    print(f"[Saved] {args.output_dir / 'feature_46_master.csv'} ({len(master):,} rows)")
